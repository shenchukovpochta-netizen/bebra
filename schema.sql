...
-- Каталог работ кладёт прайс владельца - блок «прайс сервиса» в конце
-- файла: у него две цены на строку и своя версия.
...
-- ─────────────── прайс сервиса: арендатор и сторонний ───────────────
--
-- У сервиса два прайса (регламент № 11): арендатору - за поломку по
-- его вине на нашем велосипеде, стороннему заказчику - за ремонт его
-- техники. Цены разные, работы во многом те же, поэтому у вида работ
-- два листа в одной строке: работа и запчасть арендатору, работа и
-- запчасть стороннему. Пустая работа листа (null) значит «в этом прайсе
-- такой работы нет»: наряд тогда не подставит цену, а попросит вписать
-- её руками. Ноль - это честный ноль: колодки в рамках ТО.
--
-- Лист выбирается по объекту наряда, а не по плательщику: свой
-- велосипед - прайс арендатора, чужая техника - прайс стороннего.
-- Кто платит за ремонт своего велосипеда (мы или клиент), на цену в
-- прайсе не влияет - влияет на то, выставят ли её вообще.

alter table crm.work_types alter column price drop not null;
alter table crm.work_types add column if not exists parts_price numeric(12,2) not null default 0;
alter table crm.work_types add column if not exists price_ext numeric(12,2);
alter table crm.work_types add column if not exists parts_price_ext numeric(12,2) not null default 0;

-- Прайс владельца приезжает одним заходом на версию: отметка в
-- crm.settings говорит, что эта редакция уже легла, и правки в панели
-- после неё схема не трогает. «on conflict do nothing», как у тарифов,
-- здесь не годится: старый сид клал заглушки с выдуманными ценами, и
-- «Замена контроллера» за 900 ₽ осталась бы навсегда. Заглушки, которых
-- никто не правил, выключаются - их можно включить обратно в панели.
-- Новая редакция прайса - новая дата в двух местах этого блока.
do $$
begin
  if not exists (select 1 from crm.settings
                  where key = 'work_price_version' and value = '2026-08-17') then
    update crm.work_types set active = false
     where price_ext is null and (title, category, price) in (
      ('Замена АКБ', 'Электрика', 200),
      ('Диагностика электрики', 'Электрика', 600),
      ('Замена дисплея', 'Электрика', 500),
      ('Замена зарядного устройства', 'Электрика', 150),
      ('Замена контроллера', 'Электрика', 900),
      ('Замена мотор-колеса', 'Электрика', 1500),
      ('Замена камеры', 'Ходовая', 200),
      ('Замена покрышки', 'Ходовая', 250),
      ('Замена тормозных колодок', 'Тормоза', 300),
      ('Прокачка тормозов', 'Тормоза', 400),
      ('Замена тормозного диска', 'Тормоза', 350),
      ('Правка колеса', 'Ходовая', 400),
      ('Замена цепи', 'Ходовая', 300),
      ('Техобслуживание', 'ТО', 700),
      ('Замена фары', 'Свет', 200),
      ('Замена подножки', 'Прочее', 150));
    insert into crm.work_types
      (title, category, minutes, price, parts_price, price_ext, parts_price_ext,
       node, sort) values
  ('Замена / установка держателя для телефона', 'Передняя часть', 0, 600, 800, 600, 1200, 'phone_holder', 10),
  ('Пайка держателя телефона', 'Передняя часть', 30, 300, 0, null, 0, 'phone_holder', 20),
  ('Протяжка держателя телефона через косу проводов', 'Передняя часть', 0, null, 0, 1000, 1200, 'phone_holder', 30),
  ('Замена / установка блока управления', 'Передняя часть', 0, 800, 1200, 1100, 1900, 'wiring', 40),
  ('Пайка блока управления', 'Передняя часть', 30, 1200, 0, 1600, 0, 'wiring', 50),
  ('Гидроизоляция блока управления', 'Гидроизоляция', 60, null, 0, 800, 0, 'wiring', 60),
  ('Замена / установка грипсы', 'Передняя часть', 0, 0, 300, 100, 350, 'handlebar', 70),
  ('Замена / установка руля', 'Передняя часть', 0, 450, 2000, 800, 2200, 'handlebar', 80),
  ('Замена / установка выноса руля', 'Передняя часть', 0, 450, 1700, null, 0, 'handlebar', 90),
  ('Замена / установка ручки газа', 'Передняя часть', 0, 700, 1800, 1400, 2200, 'throttle', 100),
  ('Пайка ручки газа', 'Передняя часть', 30, 800, 0, 1500, 0, 'throttle', 110),
  ('Замена зеркал, шт', 'Передняя часть', 0, 0, 500, 100, 900, 'mirrors', 120),
  ('Замена рулевой колонны', 'Передняя часть', 0, 400, 700, 1000, 1500, 'headset', 130),
  ('Замена амортизационной вилки', 'Передняя часть', 0, 2000, 6500, 2000, 6500, 'fork', 140),
  ('Замена подшипников амортизационной вилки', 'Передняя часть', 0, 800, 600, 1000, 800, 'fork', 150),
  ('Замена подшипников руля', 'Передняя часть', 0, 650, 850, null, 0, 'headset', 160),
  ('Замена / установка траверсы', 'Передняя часть', 0, null, 0, 500, 3500, 'fork', 170),
  ('Замена / установка корзинки', 'Передняя часть', 0, 200, 1600, null, 0, 'other', 180),
  ('Замена / установка переднего крыла', 'Передняя часть', 0, 300, 800, 600, 1400, 'fender_front', 190),
  ('Замена / установка крепёжной дуги переднего крыла', 'Передняя часть', 0, 400, 400, 300, 550, 'fender_front', 200),
  ('Замена / установка сигнального гудка', 'Передняя часть', 0, 350, 750, 450, 1100, 'horn', 210),
  ('Замена / установка передней фары', 'Передняя часть', 0, 600, 1400, 750, 1500, 'headlight', 220),
  ('Установка передней фары с протяжкой через косу проводов', 'Передняя часть', 0, null, 0, 1400, 1500, 'headlight', 230),
  ('Пайка фары', 'Передняя часть', 30, 600, 0, 900, 0, 'headlight', 240),
  ('Замена / установка поворотника, шт', 'Передняя часть', 0, 800, 1000, 700, 1300, 'turn_signals', 250),
  ('Пайка поворотника', 'Передняя часть', 30, 800, 0, 700, 0, 'turn_signals', 260),
  ('Замена / установка переднего гидравлического тормоза в сборе', 'Передняя часть', 0, 500, 2500, 750, 3000, 'brake_line', 270),
  ('Прокачка гидролинии переднего тормоза', 'Передняя часть', 0, 400, 0, 800, 0, 'brake_line', 280),
  ('Замена суппорта переднего тормоза', 'Передняя часть', 0, 450, 1200, 800, 2400, 'brake_line', 290),
  ('Осмотр уровня тормозной жидкости и долив', 'Передняя часть', 0, 0, 0, null, 0, 'brake_line', 300),
  ('Замена тормозных колодок перед / зад (в рамках ТО)', 'Передняя часть', 0, 0, 0, null, 0, 'brake_pads', 310),
  ('Замена тормозных колодок перед / зад (не в рамках ТО)', 'Передняя часть', 0, 200, 500, 450, 700, 'brake_pads', 320),
  ('Замена оси переднего колеса', 'Передняя часть', 0, 250, 600, 500, 850, 'wheel_front', 330),
  ('Замена переднего колеса в сборе с покрышкой', 'Передняя часть', 0, 250, 3800, 600, 5000, 'wheel_front', 340),
  ('Замена обода переднего колеса', 'Передняя часть', 0, 400, 2600, 1800, 2400, 'wheel_front', 350),
  ('Замена соска бескамерки, переднее колесо', 'Передняя часть', 0, 200, 300, 600, 300, 'tube_tire', 360),
  ('Шиномонтаж переднего камерного колеса с установкой камеры', 'Передняя часть', 0, 850, 750, 1600, 900, 'tube_tire', 370),
  ('Шиномонтаж переднего колеса с бескамерной всесезонной покрышкой', 'Передняя часть', 0, 1000, 2000, 1800, 2600, 'tube_tire', 380),
  ('Замена седла', 'Задняя часть', 0, 200, 1400, 300, 1800, 'saddle', 390),
  ('Замена подседельного штыря', 'Задняя часть', 0, 200, 1300, 400, 1800, 'seatpost', 400),
  ('Замена / установка багажника', 'Задняя часть', 0, 100, 2100, 250, 2200, 'rack', 410),
  ('Замена контроллера', 'Задняя часть', 0, 1000, 3500, 2500, 4500, 'controller', 420),
  ('Пайка фазной проводки контроллера', 'Задняя часть', 30, 2500, 0, 2500, 0, 'wiring', 430),
  ('Замена охранной сигнализации', 'Задняя часть', 0, 500, 1400, 400, 1600, 'other', 440),
  ('Замена задней фары (стоп-сигнала)', 'Задняя часть', 0, 200, 1400, 500, 1500, 'taillight', 450),
  ('Снятие / установка защиты цепи', 'Задняя часть', 0, 200, 1400, 300, 1400, 'chain_guard', 460),
  ('Снятие / установка цепи', 'Задняя часть', 0, null, 0, 250, 350, 'chain_guard', 470),
  ('Замена / установка подножки', 'Задняя часть', 0, 400, 1900, 700, 2600, 'kickstand', 480),
  ('Замена / установка пружинки подножки, шт', 'Задняя часть', 0, 100, 250, 150, 300, 'kickstand', 490),
  ('Замена / установка заднего крыла', 'Задняя часть', 0, 550, 950, 600, 1500, 'fender_rear', 500),
  ('Замена / установка крепёжной дуги заднего крыла', 'Задняя часть', 0, 200, 450, 300, 500, 'fender_rear', 510),
  ('Замена / установка заднего гидравлического тормоза в сборе', 'Задняя часть', 0, 500, 1600, null, 0, 'brake_line', 520),
  ('Замена суппорта заднего тормоза', 'Задняя часть', 0, 500, 1200, null, 0, 'brake_line', 530),
  ('Замена мотор-колеса с шиномонтажом', 'Мотор-колесо и шиномонтаж', 120, 2000, 15000, 2000, 13000, 'motor_wheel', 540),
  ('Замена датчиков Холла', 'Мотор-колесо и шиномонтаж', 0, 4000, 300, 3500, 300, 'hall_sensors', 550),
  ('Замена провода питания мотор-колеса', 'Мотор-колесо и шиномонтаж', 0, 3000, 1700, 2500, 1300, 'wiring', 560),
  ('Зачистка мотор-колеса от воды, влаги и ржавчины', 'Мотор-колесо и шиномонтаж', 0, null, 0, 2500, 0, 'motor_wheel', 570),
  ('Шиномонтаж заднего колеса с установкой камеры', 'Мотор-колесо и шиномонтаж', 0, 1300, 600, 800, 1900, 'tube_tire', 580),
  ('Шиномонтаж заднего колеса с бескамерной всесезонной покрышкой', 'Мотор-колесо и шиномонтаж', 0, 1200, 2000, 600, 1900, 'tube_tire', 590),
  ('Замена соска бескамерки, заднее колесо', 'Мотор-колесо и шиномонтаж', 0, 200, 200, 600, 300, 'tube_tire', 600),
  ('Гнездо зарядки аккумулятора', 'Аккумуляторы', 0, 300, 400, 400, 500, 'battery', 610),
  ('Замена крышки (верх / низ) аккумулятора', 'Аккумуляторы', 0, 500, 1300, 400, 1500, 'battery', 620),
  ('Замена личинки ключа аккумулятора', 'Аккумуляторы', 0, 300, 900, 500, 1400, 'battery', 630),
  ('Замена разъёма зарядки аккумулятора (папа / мама)', 'Аккумуляторы', 0, 300, 900, 600, 1200, 'battery', 640),
  ('Повреждения корпуса аккумулятора (сильные царапины и пр.)', 'Аккумуляторы', 0, 0, 750, null, 0, 'battery', 650),
  ('Замена ручки крышки аккумулятора', 'Аккумуляторы', 0, 150, 500, 200, 550, 'battery', 660),
  ('Гидроизоляция аккумулятора', 'Гидроизоляция', 60, null, 0, 1200, 0, 'battery', 670),
  ('Гидроизоляция полная (колхозников)', 'Гидроизоляция', 270, null, 0, 3500, 0, 'other', 680),
  ('Гидроизоляция полная + расширенная для контроллера (колхозников)', 'Гидроизоляция', 270, null, 0, 4500, 0, 'other', 690),
  ('Гидроизоляция полная (монстров)', 'Гидроизоляция', 270, 3500, 0, 4500, 0, 'other', 700),
  ('Гидроизоляция полная + расширенная для контроллера (монстров)', 'Гидроизоляция', 270, null, 0, 5500, 0, 'other', 710),
  ('Гидроизоляция контроллера стандартная', 'Гидроизоляция', 60, null, 0, 1500, 0, 'controller', 720),
  ('Гидроизоляция контроллера расширенная', 'Гидроизоляция', 60, null, 0, 2500, 0, 'controller', 730),
  ('Гидроизоляция мотор-колеса', 'Гидроизоляция', 60, null, 0, 2500, 0, 'motor_wheel', 740),
  ('Заварить раму', 'Рама и резьба', 0, null, 0, 2500, 0, 'frame', 750),
  ('Замена рамы (трещины / поломка)', 'Рама и резьба', 0, 3500, 9000, 3500, 11000, 'frame', 760),
  ('Прогонка резьбовых соединений, шт', 'Рама и резьба', 20, null, 0, 500, 0, 'other', 770),
  ('Нарезка резьбового соединения, шт', 'Рама и резьба', 20, null, 0, 800, 0, 'other', 780),
  ('Клеммирование проводов, шт', 'Рама и резьба', 20, null, 0, 500, 0, 'wiring', 790),
  ('Замена фишек провода, шт', 'Рама и резьба', 20, null, 0, 500, 0, 'wiring', 800),
  ('Замена / установка ручки газа (Minako)', 'Minako', 0, 600, 1800, null, 0, 'throttle', 810),
  ('Замена / установка блока управления (Minako)', 'Minako', 0, 700, 1500, null, 0, 'wiring', 820),
  ('Замена / установка контроллера (Minako)', 'Minako', 0, 1500, 4500, null, 0, 'controller', 830),
  ('Замена / установка бортового компьютера (Minako)', 'Minako', 0, 500, 2600, null, 0, 'wiring', 840),
  ('Погнутый обод мотор-колеса, за сторону', 'Штрафы и порча имущества', 0, 2000, 0, null, 0, 'motor_wheel', 850),
  ('Замена обода мотор-колеса (выпрямление невозможно)', 'Штрафы и порча имущества', 0, 1500, 5000, null, 0, 'motor_wheel', 860),
  ('Потеря аккумулятора', 'Штрафы и порча имущества', 0, 0, 24000, null, 0, 'battery', 870),
  ('Потеря (порча) зарядника', 'Штрафы и порча имущества', 0, 0, 3000, null, 0, 'charger', 880),
  ('Потеря курьерской сумки', 'Штрафы и порча имущества', 0, 0, 2000, null, 0, null, 890),
  ('Потеря зимних перчаток', 'Штрафы и порча имущества', 0, 0, 2000, null, 0, null, 900),
  ('Порча зимних перчаток (от 500 ₽ по оценке)', 'Штрафы и порча имущества', 0, 0, 500, null, 0, null, 910),
  ('Потеря дождевика', 'Штрафы и порча имущества', 0, 0, 1500, null, 0, null, 920),
  ('Потеря / порча стяжек для сумки', 'Штрафы и порча имущества', 0, 0, 300, null, 0, null, 930),
  ('Повреждение фирменного брендирования (наклейки)', 'Штрафы и порча имущества', 0, 0, 700, null, 0, null, 940),
  ('Небольшие повреждения багажника (погнут и пр.)', 'Штрафы и порча имущества', 0, 0, 1000, null, 0, 'rack', 950),
  ('Порезы на сидении', 'Штрафы и порча имущества', 0, 300, 1400, null, 0, 'saddle', 960),
  ('Замена (потеря) брелка дистанционного включения', 'Штрафы и порча имущества', 0, 800, 1200, null, 0, 'other', 970),
  ('Мойка электровелосипеда', 'Штрафы и порча имущества', 0, 400, 0, null, 0, null, 980),
  ('Мойка электровелосипеда, если оставлен отзыв', 'Штрафы и порча имущества', 0, 150, 0, null, 0, null, 990),
  ('Плановое ТО (каждые 2 недели)', 'ТО', 60, 0, 0, null, 0, 'other', 1000)
    on conflict (title) do update set
      category = excluded.category, minutes = excluded.minutes,
      price = excluded.price, parts_price = excluded.parts_price,
      price_ext = excluded.price_ext, parts_price_ext = excluded.parts_price_ext,
      node = excluded.node, sort = excluded.sort, active = true;
    insert into crm.settings (key, value, updated_by)
    values ('work_price_version', '2026-08-17', 'schema')
    on conflict (key) do update
      set value = excluded.value, updated_at = now(), updated_by = 'schema';
  end if;
end
$$;

-- ─────────────── блокировка мотора через StarLine ───────────────
--
-- Команда устройству живёт в очереди, а не уходит из панели напрямую:
-- панель в интернет не ходит, а у опроса в процессе бота уже есть и
-- авторизация StarLine, и расписание. Оператор ставит команду, опрос
-- относит её в StarLine на ближайшем круге и записывает ответ. Сама
-- блокировка у StarLine срабатывает не на ходу: мотор перестаёт тянуть
-- после остановки велосипеда, поэтому команда с задержкой в минуты
-- ничего не ухудшает.
--
-- Одна команда в очереди на трекер: «заблокировать» и «снять» подряд
-- до отправки - это спор, который StarLine решит в неизвестном порядке.

alter table crm.trackers add column if not exists blocked boolean not null default false;
alter table crm.trackers add column if not exists blocked_at timestamptz;
alter table crm.trackers add column if not exists blocked_by text;

create table if not exists crm.tracker_commands (
  id           bigserial primary key,
  tracker_id   bigint      not null references crm.trackers (id) on delete cascade,
  command      text        not null,          -- block|unblock
  alert_id     bigint      references crm.tracker_alerts (id),
  note         text,
  requested_by text,
  requested_at timestamptz not null default now(),
  sent_at      timestamptz,                    -- когда опрос отнёс её в StarLine
  ok           boolean,                        -- ответ StarLine
  result       text
);
create unique index if not exists tracker_commands_one_pending
