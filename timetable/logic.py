...
def parse_week(raw: str) -> int | None:
    """Номер учебной недели из аргумента команды. None - если это не число.

    Разбираем самим int, а не проверкой isdigit: «--5» и надстрочная «²»
    проходят isdigit, но int на них падает - и хендлер молча умирал, не
    ответив пользователю ничего.
    """
    try:
        return int(raw)
    except ValueError:
...
def display_week(today: date) -> int:
    """Учебная неделя для ответов «на эту неделю», прижатая к семестру.

    В воскресенье берётся уже следующая неделя: пар в воскресенье нет,
    неделя фактически кончилась, и показывать её - показывать прошлое.
    Вечером воскресенья кнопка «Понедельник» иначе отвечала бы расписанием
    понедельника уже прошедшего, да ещё и другой чётности.

    Вне семестра «текущей недели» нет, но показать что-то надо: до
    1 сентября это первая неделя, после конца - последняя.
    """
    if today.weekday() == 6:
        today += timedelta(days=1)
    return min(max(week_number(today), 1), LAST_WEEK)
