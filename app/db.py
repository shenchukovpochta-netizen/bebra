...
class Database:
    ...
    async def clear_anketa(self, tg_id: int) -> None:
        ...
        Основной бот анкету после подписи акта больше не стирает - она нужна
        повторным арендам и живёт до ретеншена (clear_files). Метод остаётся
        для MAX-бота со старым потоком: там договор подписан, зафиксирован,
        и дальше данные не нужны. Реквизиты самого договора (номер, отпечаток,
        момент подписания) остаются: без них нечем доказать, что подписано.
        """
        await self.pool.execute(
