...
async def st_close_reason(message: Message, bot: Bot, db: Database, cfg: Config,
    ...
    if text.lower() == "отмена":
        await db.patch(user["tg_id"], expected_state=logic.WAIT_CLOSE_REASON,
                       state=logic.APPROVED)
...
async def cb_return_sign(callback: CallbackQuery, bot: Bot, db: Database,
    ...
    closed = dict(data.get("return_data") or {})
    await db.log_event(tg_id, "rental_closed", {
