...
OPS_TOPIC_FIX="${OPS_TOPIC_FIX:-7}"
OPS_TOPIC_RETURN="${OPS_TOPIC_RETURN:-2383}"
OPS_TOPIC_DEBT="${OPS_TOPIC_DEBT:-23488}"
OPS_TOPIC_GPS="${OPS_TOPIC_GPS:-23745}"
OPS_TOPIC_DAILY="${OPS_TOPIC_DAILY:-604}"
EOF
chmod 600 .env
ok ".env готов"
...
cat <<EOF
    ...
    Веб-панель CRM (клиенты, парк, аренды, деньги) слушает 127.0.0.1:8080
    на сервере. С вашего компьютера:
      ssh -L 8080:127.0.0.1:8080 root@$(hostname -I 2>/dev/null | awk '{print $1}')
