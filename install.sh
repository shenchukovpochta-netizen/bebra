...
# ─── 7. Оферта ──────────────────────────────────────────────────────────────
bold "7/8. Оферта"
ask OFERTA_URL "Ссылка на публичную оферту:" '^https?://.+' "нужна ссылка http(s)://…"
ask OFERTA_VERSION "Редакция оферты (дата):" '^[0-9]{4}-[0-9]{2}-[0-9]{2}$' \
    "формат ГГГГ-ММ-ДД, например 2026-01-15"
...
# Шаблон договора остался рыбой - это самая частая незамеченная недоделка:
# бот работает, договоры уходят, а в них реквизиты арендодателя прочерками.
if grep -q "ЗАМЕНИТЕ ЭТОТ РАЗДЕЛ" app/contract_template.md 2>/dev/null; then
  printf '\n\033[33m  ВНИМАНИЕ: текст договора не заполнен.\033[0m\n'
  cat <<'EOF'
    В app/contract_template.md сейчас рыба: реквизиты арендодателя прочерками,
    раздел 5 про персональные данные — заглушка. Бот уже работает и будет
    выдавать договоры в таком виде.

      nano app/contract_template.md
