...
compose, а uvicorn по умолчанию верит заголовкам лишь от `127.0.0.1`.
...
`parse_device` переименовывает координаты сразу: у StarLine `x` это долгота, `y` широта,
перепутать их значит увезти весь парк не туда. Блокировка мотора: `block_motor` вызывает
