...
задевает все три процесса сразу, а пересборка нужна одна. Четвёртая, необязательная роль -
`python -m app.demo`, демо-стенд для франшизы: та же панель на своей базе с вымышленными
данными (профиль `demo`).
...
| `crm-demo` | та же панель в режиме демо (`python -m app.demo`): сид при старте и в 04:00 по Москве, живой день, без бота | профиль `demo`, только сеть `demo`, `mem_limit` 768m, `/tmp` в памяти; наружу `${DEMO_BIND:-127.0.0.1}:${DEMO_PORT:-8081}` |
...
`postgres-demo` - по своей сети `demo`, из которой боевых `postgres`, `crm` и бота не видно;
в обеих сетях только Caddy. Это не договорённость, а проверка: пункт «2а» `consistency.py`
роняет деплой, если в блок демо скопировали боевой секрет, том, каталог сервера, общую сеть,
`extends`/`<<`/`volumes_from`/`env_file` или хост базы. `crm-demo` нет в `depends_on` у
Caddy: без профиля `demo` compose иначе не поднял бы и HTTPS боевой панели. `bootstrap.sh`
не пускает `DEMO_DOMAIN`, равный `CRM_DOMAIN`, и `DEMO_PORT` на порту панели.
...
Режим демо (`cfg.demo`) из окружения не читается: его ставит только `app.demo`
(`runtime.demo_config`). Тогда под `BodyLimit` встаёт `DemoGate` - `X-Robots-Tag: noindex,
nofollow` на каждом ответе, 503 «Демо обновляется» на время сброса, пока
`app.state.maintenance`, предел запросов с адреса (`app.state.demo_limits`, `DemoLimits`: 4
одновременно, 3 в секунду с запасом 40, статика не считается) и `demo-` в имени любого
скачанного файла; в `auth` после проверки входа POST по `demo_blocked(method, path)`
получает «В демо-версии это недоступно» без записи (в списке и `/import`); `/robots.txt`
публичен только в демо (`DEMO_PUBLIC`), cookie называется `crm_demo`, замок входа по логину
выключен (по адресу остаётся). Ещё в демо: выгрузки с пометкой над шапкой и не длиннее
`DEMO_EXPORT_ROWS`, «Скачать наш» шаблон скрыт и отвечает 404 (`our_template`: в файлах
реквизиты настоящего ИП), фото номера на «Вводе техники» не включается (`intake_save`). При
`demo=False` ничего из этого нет, кроме `BodyLimit` и потоков.
...
Демо-стенд в этой базе не участвует вовсе: у `crm-demo` свой Postgres, и схему он не
«применяет», а сносит и накатывает заново вместе с сидом (`app.demo.seed.reset`, одна
транзакция под тем же `pg_advisory_xact_lock(7331)`). Перед первым `DROP`
`runtime.check_database` проверяет, что база именно демо: имя с `demo` и ни одного
сотрудника без логина `demo`.
...
`_env()` считает пустую строку отсутствием значения, потому что compose подставляет `""`
для любой незаданной `${VAR}`.
...
| `app/demo/seed.py` | `reset()`: снести схемы, накатить `schema.sql`, засеять - одной транзакцией; сотрудники демо `STAFF`, реквизиты, `summary()` с тремя числами |
| `app/demo/core.py` | ядро сида: точки, закупки, парк и день за днём аренды, деньги, касса, журналы статусов и мест; `later_today` - остаток сегодняшнего дня (открытие касс, платежи) для живого дня |
...
| какие ключи и каталоги у демо | `runtime.demo_config` в `app/demo/runtime.py`; секреты, тома, ключи и сети демо в compose - только вместе с белым списком (`DEMO_SECRETS`, `DEMO_VOLUMES`, `DEMO_KEYS`) в `consistency.py` |
...
в `pyproject.toml`, и прогон не должен требовать ничего, кроме Python. 83 файла
`tests/test_*.py`, 2535 тестов, полный прогон около девяти с половиной минут (полторы из
них - сид и обход демо-стенда на живом Postgres).
...
| демо-стенд: обход панели | `tests/test_demo_crawl.py` | `pgserver`: сид через `runtime.reset`, каждая GET-страница и ссылка под тремя логинами - только 200, разделы не пусты (отчёты «с начала месяца» - за 30 дней), выгрузки помечены, сид < 60 с |
| демо-стенд: режим панели | `tests/test_demo_mode.py` | заглушка базы: плашка, логины, закрытые POST (и импорт), замок входа, noindex и 503, предел тела и темпа, пометка выгрузок, наши шаблоны, фото номера, конфиг демо, ночной сброс с повтором, страж базы |
...
2. `app/config.py` против compose: прочитанная переменная, которую compose не передаёт, и
   ...
   2а. Изоляция демо-стенда, белым списком: в блоках `crm-demo` и `postgres-demo` только
   ключи `DEMO_KEYS` (никаких `extends`, `<<`, `volumes_from`, `env_file`, `network_mode`),
   секреты только `DEMO_SECRETS`, тома только `pgdata_demo` (каталог сервера - провал), сеть
   только `demo`; в общем разделе `secrets:` файлы демо - свои; `crm-demo` ходит в
   `postgres-demo` (в любой записи `POSTGRES_HOST`), а Caddy стоит в обеих сетях. Плюс
   прежний поиск боевых имён (`DEMO_FORBIDDEN`). Комментарии не считаются. Логин демо
   публичен, а сброс сносит схемы своей базы: копипаста соседнего сервиса стоила бы
   настоящих клиентов.
...
```bash
python3 -m unittest discover -s tests -q        # весь набор, ~8 минут
ruff check .
python3 consistency.py
...
каталогам (`$root`, `$app`, `$i18n`, `$handlers`, `$demo`, `$crm`, `$web`, `$webTemplates`), потому
