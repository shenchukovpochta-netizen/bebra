...
`hijack`: «антиограбление» это наша же блокировка мотора. На связи - `status == 1`;
«молчит» считается по более позднему из точки и `ts_activity` (`logic.tracker_seen_at`):
в подвале связь есть, спутников нет. Расшаренные на кабинет трекеры (`shared_devices`)
входят в тот же список. Блокировка мотора: `block_motor` вызывает
...
клиента. Пусто: выписка не тянется, панель не показывает кнопку ссылки (`acquiring()` вернёт
