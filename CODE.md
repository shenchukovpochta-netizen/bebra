...
Карточка велосипеда пишет точку через `update_bike(..., keep_rented_location=True)`: условие
`case when status = 'rented'` стоит в самом UPDATE, а не проверкой перед ним, потому что
выдача, закоммиченная между чтением карточки и записью, иначе увела бы велосипед в аренде
с точки аренды (UPDATE ждёт замка строки и перечитывает её). Переименование отказывает,
если новое имя уже стоит в записях вне справочника (`"orphan"`), и переписывает
`?location=` сохранённых фильтров той же транзакцией.

Порядок блока точек в `schema.sql` важен: точка старых аренд и переезд велосипеда идущей
аренды на неё идут ДО первого заполнения журнала мест и без строки журнала (та же отметка
`crm.location_rename`, сброшенная сразу после), чтобы первая строка истории уже стояла на
точке аренды, а не на точке из карточки. На базе с заполненным журналом блоки ничего не
переписывают: история остаётся как есть.
...
- сеть, TLS и таймаут превращаются в `AvitoError`, а 200 с пустым или нечитаемым телом у
  чтения - тоже ошибка: пустой ответ «сообщений нет» сдвинул бы курсор чата за
  непрочитанное. У отправки наоборот: 200 значит «ушло», повтор был бы дублем;
...
(`inbox_threads.ext_cursor`; у чата без обращения - `settings.inbox_avito_seen`), читает
список страницами, пока страница меняется, и берёт сообщения не старше позднего из двух:
первого удачного круга минус сутки (`inbox_avito_since`) и срока хранения - иначе опрос
возвращал бы переписку, удалённую чисткой. Курсор не двигается за сообщение, которое не
записалось. Своё сообщение из приложения Авито пишется исходящим и снимает ожидание.
